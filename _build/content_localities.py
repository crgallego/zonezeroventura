...
VCFD = ("Ventura County Fire Department",
        "The Ventura County Fire Department (VCFD) is the Authority Having Jurisdiction here. The state Zone 0 framework is the floor; VCFD's adopted schedule and programs set the local specifics.")
...
loc("camarillo", "Camarillo", "city", "Very High",
    *VCFD,
    "Camarillo's hazard story changed in November 2024, and everyone here knows it. The city carries a real, current Very High designation on its hillside edges, the Camarillo Heights neighborhood above the valley floor most of all, and the Mountain Fire is the reason the maps and the community now read those hills the same way. Zone Zero (Zone 0) applies to parcels within the mapped zones, and this is a city where the requirement needs no abstract justification.",
    ...
    [("Is Camarillo in a Very High fire hazard zone?",
     ...
     ("Who enforces Zone Zero in Camarillo?",
      "The Ventura County Fire Department, serving the city under contract, applies the state framework locally.")],
...
loc("santa-paula", "Santa Paula", "city", "designation under review on this site (tier not fully confirmed this pass)",
    *VCFD,
    ...
    [("Verified", "VCFD contract city in the Santa Clara River valley"), ("2017", "near the Thomas Fire's ignition, hit hard in its opening hours"), ("Unconfirmed", "current FHSZ tier not asserted on this site this pass"), ("Free check", "your parcel's actual tier is one assessment away")],
...
loc("fillmore", "Fillmore", "city", "designation under review on this site (tier not fully confirmed this pass)",
    "Ventura County Fire Protection District (Station 27)",
    "Fillmore is served by the Ventura County Fire Protection District through Station 27, the same VCFD family that serves most of the county. The state Zone 0 framework is the floor; the district's adopted specifics apply locally.",
    "Fillmore sits up the Santa Clara River valley from Santa Paula, citrus country against the Topatopa foothills, served by the Ventura County Fire Protection District's Station 27. Like its downriver neighbor, it's included here for its verified fire history and service area, and caveated because this site could not fully confirm the city's current Fire Hazard Severity tier this research pass. What's confirmed is stated; what isn't is labeled, and your parcel's actual answer is a free check away.",
    "The valley the Thomas Fire ran through",
    "The Thomas Fire's December 2017 run moved through the Santa Clara River valley corridor, and regional coverage places Fillmore among the communities affected as the fire spread from its ignition area. That verified regional history is why the city is on this site. The city's exact current FHSZ tier under the 2025 maps is the piece we have not confirmed, and this page deliberately doesn't assert it. The foothill edge above town is the geography most worth checking at the parcel level, which the free assessment does definitively.",
    [("Verified", "VCFPD Station 27 service, Santa Clara River valley"), ("2017", "in the valley corridor of the Thomas Fire's spread"), ("Unconfirmed", "current FHSZ tier not asserted on this site this pass"), ("Foothills", "the Topatopa edge is the geography worth checking")],
    [("Is Fillmore in a Very High fire hazard zone?",
      "We haven't confirmed the city's current tier with full certainty, so we won't assert it. What's verified: Fillmore sits in the Thomas Fire's 2017 valley corridor and is served by the Ventura County Fire Protection District's Station 27. The definitive answer for your property is a free parcel-level check."),
     ("Who is Fillmore's fire authority?",
      "The Ventura County Fire Protection District through Station 27, part of the same county fire family (VCFD) that serves most of Ventura County."),
     ("What should a Fillmore homeowner actually do?",
      "Get the free parcel check. If your parcel is in a mapped zone, the standard Zone 0 path applies: fence line first, then the rest of the five-foot ring. If it isn't, the ember-aware basics remain cheap insurance in citrus-and-foothill country.")],
...
loc("meiners-oaks", "Meiners Oaks", "unincorporated community", "Very High (Ojai Valley zone)",
    *VCFD,
    "Meiners Oaks sits at the Ojai Valley's western end, an unincorporated community under the oaks that give it its name, inside the valley's confirmed Very High zone and served by the Ventura County Fire Department. Zone Zero (Zone 0) applies to structures here the same as in Ojai proper, and the community shares the valley's defining tension: the canopy is the identity, and the five-foot ring at the structure, not tree removal, is the requirement.",
    ...
    [("Does Zone Zero apply in Meiners Oaks?",
      "Yes. Meiners Oaks sits inside the Ojai Valley's confirmed Very High zone as an unincorporated community under the Ventura County Fire Department, so ember-resistant zone requirements apply to structures here."),
     ("Will the rules require removing the community's oaks?",
      "No. Local rules may allow well-maintained trees, and the state draft scaled back blanket vegetation restrictions. The clear requirement is a noncombustible first five feet at each structure; verify tree specifics with VCFD."),
     ("What's the right first project on a Meiners Oaks parcel?",
      "The five-foot ring: firewood and stored fuel out, bark mulch swapped for gravel at the foundation, and the fence section touching the house evaluated. Then the free assessment for the full picture.")],
    "under the oaks, inside the zone"),

loc("oak-view", "Oak View", "unincorporated community", "Very High (Ojai Valley zone)",
    *VCFD,
    "Oak View lines Highway 33 between Ojai and the coast, the valley's corridor community, unincorporated, inside the Ojai Valley's confirmed Very High zone, and served by the Ventura County Fire Department. Zone Zero (Zone 0) applies to structures here, and the community's corridor geography, homes stepping up from the highway toward brushy slopes on both sides, makes the parcel-level five feet an especially concrete conversation.",
    "The corridor's shared 2017 memory",
    "Oak View shared the Ojai Valley's Thomas Fire experience in December 2017: the fire burned the surrounding terrain while Highway 33, the corridor's lifeline, carried the valley's evacuations. For a community strung along one road between slopes, the lesson was about both preparation and egress, and the preparation half is what each parcel controls. The ember-resistant ring at every structure, house, garage, and outbuilding alike, is the requirement; the slopes beyond five feet fall under the broader defensible-space zones the free assessment also covers.",
    ...
    [("Does Zone Zero apply in Oak View?",
      "Yes. Oak View sits inside the Ojai Valley's confirmed Very High zone as an unincorporated community under the Ventura County Fire Department, so ember-resistant zone requirements apply to structures here."),
...
def build(write_page):
    for d in L:
        ...
        desc = (f"Zone Zero (Zone 0) requirements for {d['name']}: {d['ahj']} enforcement, "
                f"local fire context, and the ember-resistant zone rules that apply.")
        snaps = "\n".join(
        ...
        qa_tail = ("Zone Zero requires an ember-resistant zone in the first 0 to 5 feet around structures in the mapped Very High areas, with the combustible-fence restriction taking effect immediately upon adoption of the final regulations."
                   if is_vh else
                   "This site states the city's verified fire history and service area while deliberately not asserting an unconfirmed hazard tier; a free parcel-level check gives the definitive answer for your property.")
...
<p>{d['ahj_note']} Deadlines pages carry the state defaults; <a href="/deadlines/">your jurisdiction sets the specifics</a>, and this page names yours. For the requirements themselves, start with <a href="/regulations/">the regulations guide</a>, <a href="/fences/">fences and gates</a>, and <a href="/landscaping/">landscaping</a>.</p>
</div></section>
"""
